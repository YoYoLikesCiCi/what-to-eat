...
Future<int> DeleteFoodServer(Map data)async{
    Response responsE;
    Dio dio = Dio();
//    print(data);
    //0都为网络错误
    //1都为成功
    responsE = await dio.post('http://47.93.25.50/deletefood',data:data);
    print(responsE.statusCode);
    if(responsE.statusCode == 200){
//        print(responsE.data.toString());
        if(responsE.data.toString() == 'success'){
...
Future<int> PlusTimesServer(Map data)async{
    Response responsE;
    Dio dio = Dio();
//    print(data);
    //0都为网络错误
    //1都为成功
    responsE = await dio.post('http://47.93.25.50/plustimes',data:data);
//    print(responsE.statusCode);
    if(responsE.statusCode == 200){
//        print(responsE.data.toString());
        if(responsE.data.toString() == 'success'){
